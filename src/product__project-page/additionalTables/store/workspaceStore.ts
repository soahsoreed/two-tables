...
import {INSERT_WORKSPACE_ONE} from "../api/workspase/createWorkspace.ts";
import {DELETE_WORKSPACE} from "../api/workspase/deleteWorkspace.ts";

export const useWorkspace = create<CustomAdditionalStore<IAdditionalData>>(
  (set) => ({
    data: null,
    error: null,
    loading: true,
    ...
    async createData(obj) {
      ...
      try {
        ...
        if (res.data.insert_workspaces_one) {
    ...
    deleteData: async (id) => {
      try {
        ...
        if (res.data.update_workspaces_by_pk) {
