...
import {client} from "../../../apollo/client.ts";import {INSERT_EDU_MATERIAL_ONE} from "../api/learningMaterials/createLearningMaterial.ts";
...
import {DELETE_EDU_MATERIAL} from "../api/learningMaterials/deleteLearningMaterial.ts";

export const useLearningMaterials = create<CustomAdditionalStore<IAdditionalData>>(
  (set) => ({
    data: null,
    error: null,
    loading: true,
    ...
    async createData(obj) {
      ...
      const options = {
        ...
        mutation: INSERT_EDU_MATERIAL_ONE,
        variables: {
      ...
      try {
        ...
        if (res.data.insert_edu_materials_one) {
