...
import {useMarketingMaterialsPageFilters} from "../../components/store/useFiltersStore.ts";
...
const MarketingMaterialsTable = () => {
  ...
  const {pageFilterObject, setPageFilterObject}  = useMarketingMaterialsPageFilters()
    ...
    <Flex style={{width:'100%'}}>
      <Flex vertical={true} style={{width:'100%', padding: '24px', background: '#F3F4F6'}}>
        <Flex style={{ padding: '16px',borderTopLeftRadius: '16px', borderTopRightRadius: '16px', background: 'white'}} justify={'space-between'} align={'center'}>
          <Typography.Title style={{margin: 0}} level={4}>Маркетинговые материалы</Typography.Title>
          <Button
            disabled={currentProdData.status == 'delete' || currentProdData.status == 'finished'}
            onClick={() => {
            setIsModalOpen(true)
        ...
        <Table pagination={{showSizeChanger: true}}
          ...
          dataSource={currentProdData.marketing_materials?.length?
            currentProdData.marketing_materials.filter(el => el.status != 'delete')
            : null} />
      </Flex>
    </Flex>
  );
