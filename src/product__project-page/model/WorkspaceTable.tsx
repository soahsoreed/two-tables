...
import { useWorkspacePageFilters} from "../../components/store/useFiltersStore.ts";
...
const WorkspaceTable = () => {
  ...
  const {pageFilterObject, setPageFilterObject}  = useWorkspacePageFilters()
    ...
    <Flex style={{width:'100%'}}>
      <Flex vertical={true} style={{width:'100%', padding: '24px', background: '#F3F4F6'}}>
        <Flex style={{ padding: '16px',borderTopLeftRadius: '16px', borderTopRightRadius: '16px', background: 'white'}} justify={'space-between'} align={'center'}>
          <Typography.Title style={{margin: 0}} level={4}>Рабочее пространство</Typography.Title>
          <Button             onClick={() => {
            setIsModalOpen(true)
          }}
          disabled={currentProdData.status == 'delete' || currentProdData.status == 'finished'}
        ...
        <Table pagination={{showSizeChanger: true}}
          ...
          dataSource={currentProdData.workspaces?.length?
            currentProdData.workspaces.filter(el => el.status != 'delete')
            : null} />
      </Flex>
    </Flex>
  );
